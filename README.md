...
- Crossover order across readers — your design choice was one pass per phase. No counterbalancing logic.
- Training-feedback loops on calibration cases — single calibration case shows the UI but is not analyzed.
- AI rendering features beyond fill + edge-only: `pulsate` is in the style schema but not yet implemented in the renderer. Hook it up in `viewer.component.ts` once you've decided on the animation.

## Known follow-ups

- I built this without execution feedback (no Tauri build in the sandbox). Expect tiny fixes on first `npm run dev` — most likely candidates are import-path mismatches in Tauri 2 (the API surface drifts), the `register_uri_scheme_protocol` signature, and the dialog plugin import paths.
- The populate script (Python) is a separate step — write it once the app builds for you.

## First-build debugging cheatsheet

Most likely places things break and where to look:

1. **`tauri::UriSchemeContext` import** — if not at the top level in your Tauri 2 version, try `tauri::webview::UriSchemeContext`. Fix in `src-tauri/src/image_protocol.rs:4`.
2. **`http::Uri::host()` returning `None`** — on some platforms the rewritten URL has no host. If hitting 400s for image URIs, log `url.scheme().unwrap_or("")`, `url.authority()`, `url.path()` at the top of `handle()`.
3. **`tauri::http::Request<Vec<u8>>` vs `Request<&[u8]>`** — Tauri 2 minor versions have wavered on this; if compile complains, match what the trait expects.
4. **Dialog plugin permissions** — if `dialog:allow-open` errors at runtime, the v2 permission names occasionally shift; try just `dialog:default` first.
5. **CSP blocking `fundus://...` images** — open the webview devtools, console errors will name the missing CSP directive. Add it to `tauri.conf.json` → `app.security.csp`.
6. **Angular strict template** — if `appState.session()!.classes` fails type-check, use the `*ngIf=" ... as s"` rebind form instead.
7. **WAL mode + read-only project DB** — if rusqlite errors about WAL on a read-only file, it's because we opened it `READ_ONLY` but SQLite tried to create a journal anyway. Add `PRAGMA query_only=1` after open, or open with `OpenFlags::SQLITE_OPEN_READ_ONLY | OpenFlags::SQLITE_OPEN_NO_MUTEX` (already set).
