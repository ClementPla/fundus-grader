...
pub fn open(path: &Path) -> Result<Connection> {
    ...
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS submissions (
            ...
            ai_influenced TEXT,             -- legacy; no longer populated
    ...
    // Migration: add AI-reveal columns to submissions if they don't exist yet.
    // Idempotent — pragma_table_info returns 0 if absent.
    let exists: i64 = conn.query_row(
        "SELECT COUNT(*) FROM pragma_table_info('submissions') WHERE name='ai_decision'",
        [],
        |r| r.get(0),
    )?;
    if exists == 0 {
    ...
    let phase: Option<String> = conn
        .query_row("SELECT value FROM admin_config WHERE key='phase'", [], |r| r.get(0))
        .ok();
    if phase.is_none() {
        conn.execute("INSERT INTO admin_config(key,value) VALUES('phase','no_ai')", [])?;
    }
